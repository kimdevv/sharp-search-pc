...
def get_daum_images_brute_force(keyword):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            combined_raw = []
            for src, alt in raw_matches:
                combined_raw.append({"src": src, "alt": alt})
            for alt, src in raw_matches_reverse:
                combined_raw.append({"src": src, "alt": alt})
            ...
            for item in combined_raw:
                ...
                if "argon" not in src and "dn/bt" not in src:
                    continue
                if "ico_" in src or "icon" in src or alt == "이미지" or not alt:
                    continue
                if any(r['imageUrl'] == src for r in results):
                    continue
    ...
    while len(results) < 2:
        results.append({
            "title": f"'{keyword}' 관련 정보 {len(results)+1}",
            "imageUrl": generic_fallback
        })
        
    return results[:3]

def create_html(keyword, search_results):
    encoded_keyword = urllib.parse.quote(keyword)
    target_url = f"https://m.search.daum.net/search?w=tot&q={encoded_keyword}"
...
<head>
    ...
    <title>Kakao Sharp Search with Preview</title>
    ...
    <style>
        ...
        .info {{ color: #888; font-size: 12px; margin-top: 20px; line-height: 1.5; }}
    </style>
</head>
<body>
    <div class="container">
...
def run_sharp_search():
    try:
        ...
        print(f"Sharp search [{keyword}] requested.")
        search_results = get_daum_images_brute_force(keyword)

        create_html(keyword, search_results)
        webbrowser.open(f"http://localhost:8000/index.html?t={int(time.time())}")
...
if __name__ == "__main__":
    ...
    print("Server Started. Press Ctrl + Shift + D to run sharp search.")

    keyboard.add_hotkey('ctrl+shift+d', run_sharp_search)
    keyboard.wait()